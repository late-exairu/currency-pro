...
export default function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 h-[60px] border-b border-slate-200 bg-white/80 py-2 backdrop-blur-sm md:h-[72px] md:py-4 lg:h-[96px] lg:py-6">
      <div className="container relative flex justify-center md:justify-between">
        ...
        >
          {menuData.map((item: menuItem) => (
            <li key={item.id}>
              <Link
                className="flex h-12 items-center justify-center px-2.5 transition-colors hover:text-blue-500"
                href={item.url}
              >
                {item.title}
              </Link>
